...
> 状态：已确认并执行（V2-A）；V2-B 待立项
> 日期：2026-08-05
> 确认：项目负责人在启动 `codex/v2-domain-migration-docs` 文档任务时确认 V2-A 边界
> 关联：[V2 大陆域名与腾讯云迁移开发交接](../FOODPRINT_V2_MAINLAND_DOMAIN_MIGRATION_DEVELOPMENT_HANDOFF_2026-08-05.md)

## 背景

Foodprint 已完成 ICP 备案，正式入口已切换到 `foodprint.com.cn` 并在腾讯云 Lighthouse 运行 Next.js。当前项目并非只使用一个可替换的数据库：它依赖 Supabase Auth、PostgreSQL、Storage、RLS、RPC 和 Edge Functions，且业务页面和 Server Actions 已直接接入这些能力。
...
1. `foodprint.com.cn` 和 Next.js 应用运行环境迁到腾讯云 Lighthouse。
...
- 因为 App Runtime 已迁移，Vercel 不再承担正式应用服务；在稳定期保留旧部署作为回滚落点。
