...
> 状态：已确认采用方案 B；历史 migration 对账已于 2026-08-03 完成。正式 Release workflow 仍保持关闭；既有 Vercel Git 自动部署暂时保留，之后再切换为 Deploy Hook。
...
2. 合入本身绝不触发数据库变更。既有 Vercel Git 集成目前仍可能为 main 生成部署；它不是正式 Release，也不会写入 Supabase。仅在 Deploy Hook 已验证后，才合入关闭 Git 自动部署的 vercel.json。
