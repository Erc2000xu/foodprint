...
P0 地图可靠性与安全基线 → V1.2 发现与去试试 → V1.3 记一顿与地点详情 → V1.4 字体与文案收束 → V2 大陆部署与地图体验 → V2.1 低延迟与 PWA 用户体验 → V2.2 启动、跳转与私有图片性能 → V2.3 发现页动态地图 → V2.4 地图优先体验与生产回归 → V2.4.1 地图控件与照片上传修复 → V2.4.2 附近优先、可扩展管理与人均回归 → V3 视觉系统与精致表达

P0 是最初的地图阻塞基线：V1.2 的地点检索入口依赖稳定高德服务。V2 已完成腾讯云和正式域名切换；V2.3 完成动态地图基础闭环，V2.4 接续解决正式使用中发现的合规、图片、Pin、布局与地图浏览体验问题，V2.4.1 再收口真机验收暴露的照片上传与地图控件可用性。V2.4.2 处理 2026-08-27 真实使用反馈中的附近定位、管理扩展性、地点预览裁切和人均数据断链；Spec 已获批准且仓库实现已落地，自动化、真机、PWA 与生产门禁仍未关闭，不替代 V2.4.1 照片 P0。V3 只负责更广泛的视觉与表达，不替代 V2.2–V2.4.2 的正确性、性能和生产验收。
...
| FOODPRINT_V2_4_1_MAP_POLISH_AND_PHOTO_UPLOAD_RECOVERY_DEVELOPMENT_HANDOFF_2026-08-18.md | 既有实现已归档；2026-08-20 真机复验仍失败 | 常规手机照片处理 / 上传 / 补传、筛选浮层裁切、定位透明 PNG srcset 与 SVG 回退、动态面板安全提示及完整门禁 | 照片 P0 以 2026-08-20 根因审计为当前实施入口 |
| FOODPRINT_V2_4_1_PHOTO_UPLOAD_ROOT_CAUSE_AND_FIX_HANDOFF_2026-08-20.md | 根因审计完成，P0 修复待实施 | 照片预处理失败、DataTransfer 静默丢图、PWA 旧版本锁定、观测失真、真实浏览器与生产门禁的修复合同 | 先完成版本对齐与 PWA 更新，再实施 Worker/WASM WebP fallback、显式 FormData 与全链路验收 |
...
| specs/2026-08-v2-4-2-nearby-management-price.md | 草案，待项目负责人批准编码 | 附近优先定位、独立管理中心、地点预览操作可达与每次到访人均/统一平均的产品技术合同 | 批准四项产品取舍；Codex M0 技术预检；与 V2.4.1 按单分支规则串行 |
...
8. V2.4.1 的照片 P0 已因 2026-08-20 真机复验失败重新打开；后续实现以根因审计交接为准，先修版本/PWA、真实 WebP fallback 与显式 FormData，再做真机和生产 canary；不得只改 CSS、质量参数或错误文案后关闭版本。
9. V2.4.2 已获项目负责人批准并完成仓库实现；自动化、干净数据库、真实高德／真机／PWA 与生产门禁仍在验证，不得用本版本关闭 V2.4.1 照片 P0。
10. V3 以 V2.2 / V2.4.1 / V2.4.2 稳定后的用户反馈和设计探索为输入，先出可点击原型/视觉基线，再实施。
