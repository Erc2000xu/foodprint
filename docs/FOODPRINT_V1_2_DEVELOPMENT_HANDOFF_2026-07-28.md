...
- 原主入口调整为“发现”，继续保留原有的图片、行政区、筛选与地点卡片；默认列表视图与地图视图在同一页面内切换。
- `/discover` 与 `/map` 仅保留为旧链接兼容跳转，不再承载第二套主页面。
...
- [发现主页面](../src/app/page.tsx)、[发现浏览组件](../src/components/map/map-browser.tsx)、[旧链接兼容页](../src/app/map/page.tsx)
...
8. 在手机宽度检查底部“发现”入口、去试试表单、验证确认框、发现页内的列表/地图切换和网络/高德错误提示。
