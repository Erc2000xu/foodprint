-- Foodprint V2.4: keep visible historical photos in the discovery index even
-- before their derived thumbnail exists. The client receives only photo ID and
-- dimensions; the server-side sign route chooses thumbnail or canonical data.
-- This forward migration replaces the function definition without editing the
-- published V2.3 migration or widening any RLS/Storage policy.
...
as $$
  ...
  selected as (
    select
      ...
      coalesce(cover.thumbnail_width, cover.width) as cover_photo_width,
      coalesce(cover.thumbnail_height, cover.height) as cover_photo_height,
      gp.created_at
    ...
    left join lateral (
      select array_agg(distinct scene_tag.scene_tag_slug order by scene_tag.scene_tag_slug) as scene_tags
      from public.place_marks mark
    ...
    left join lateral (
      select photo.id, photo.thumbnail_width, photo.thumbnail_height, photo.width, photo.height
