-- The output name place_id is a PL/pgSQL variable.  Use the named table
-- constraint in the upsert so that it cannot be parsed as that variable.
create or replace function public.save_place_mark(
  ...
  p_experience_attested boolean, p_first_visited_on date default null,
  p_last_visited_on date default null, p_short_review text default null,
  p_recommended_items text[] default '{}'::text[], p_price_per_person numeric default null,
  p_quality_rating numeric default null, p_value_rating numeric default null,
  p_environment_rating numeric default null, p_service_rating numeric default null,
  p_uniqueness_rating numeric default null, p_would_revisit public.revisit_preference default null
)
...
begin
  ...
  on conflict on constraint group_places_group_id_place_id_key do nothing
  ...
  if v_group_place_id is null then
    select existing_group_place.id into v_group_place_id
      ...
      where existing_group_place.group_id = p_group_id and existing_group_place.place_id = v_place_id
      for update;
  else
  ...
  if v_mark_id is null then
    ...
    insert into public.place_marks (group_place_id, user_id, overall_rating, quality_rating, value_rating, environment_rating, service_rating, uniqueness_rating, would_recommend, would_revisit, short_review, recommended_items, price_per_person, first_visited_on, last_visited_on, experience_attested_at)
    values (v_group_place_id, v_user_id, p_overall_rating, p_quality_rating, p_value_rating, p_environment_rating, p_service_rating, p_uniqueness_rating, p_would_recommend, p_would_revisit, nullif(trim(p_short_review), ''), coalesce(p_recommended_items, '{}'::text[]), p_price_per_person, p_first_visited_on, p_last_visited_on, now())
  ...
  update public.group_places set first_mark_id = coalesce(first_mark_id, v_mark_id), status = 'active', archived_at = null where id = v_group_place_id;
