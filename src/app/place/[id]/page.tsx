...
type PlacePhoto = { id: string; user_id: string; thumbnail_object_key: string | null; thumbnail_width: number | null; thumbnail_height: number | null; sort_order: number; visit_record_id: string | null };
...
export default async function PlaceDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const [{ data: placeRows }, { data: statsRows }, { data: marks }, { data: photos }, { data: opinionSummaries }, { data: timelineRows }, { data: businessAreaCache }] = await Promise.all([
    ...
    supabase.from("photos").select("id, user_id, thumbnail_object_key, thumbnail_width, thumbnail_height, sort_order, visit_record_id").eq("group_place_id", groupPlace.id).is("deleted_at", null).is("hidden_at", null).not("thumbnail_object_key", "is", null).order("sort_order").limit(12),
  ...
  const thumbnailRows = ((photos ?? []) as PlacePhoto[]).filter((photo) => photo.thumbnail_object_key);
  const signedResult = thumbnailRows.length ? await supabase.storage.from("place-photos").createSignedUrls(thumbnailRows.map((photo) => photo.thumbnail_object_key as string), 60 * 15) : { data: [] };
  const signedByKey = new Map((signedResult.data ?? []).map((photo) => [photo.path, photo.signedUrl]));
  const signedPhotos = thumbnailRows.map((photo) => ({ ...photo, signedUrl: signedByKey.get(photo.thumbnail_object_key as string) })).filter((photo): photo is typeof photo & { signedUrl: string } => Boolean(photo.signedUrl));
