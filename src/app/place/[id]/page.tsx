...
export default async function PlaceDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const { data: group } = await supabase.from("groups").select("name").eq("id", groupPlace.group_id).maybeSingle();
  ...
  const [{ data: place }, { data: stats }, { data: marks }, { data: photos }, { data: opinionSummaries }, { data: timeline }, { data: businessAreaCache }] = await Promise.all([
  ...
  return <AppShell groupName={group?.name}><section className="place-detail">
    <Link className="back-button" href={safeReturnTo}>← 回到发现</Link>
