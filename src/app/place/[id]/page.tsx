...
import { averageValidVisitPrices, formatAveragePrice, priceSummaryFromDatabase, visitPriceLabel, type PriceSummary } from "@/lib/price";
...
type TimelineRecord = { visit_record_id: string; visited_on: string | null; strength: number; tags: string[]; note: string | null; dishes: string[]; price_per_person?: number | string | null; created_at: string; display_name: string; can_delete: boolean };
type PlaceDetailReadModel = {
  ...
  avg_price_per_person: number | string | null;
  price_sample_count: number | string;
...
export default async function PlaceDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const { data: detailRows } = await supabase.rpc("get_group_place_detail_v2_4_2", { p_group_place_id: groupPlace.id, p_timeline_limit: 20 });
  const detail = (detailRows?.[0] as PlaceDetailReadModel | undefined) ?? null;
  const [{ data: placeRows }, { data: statsRows }, { data: marks }, { data: photos }, { data: opinionSummaries }, { data: timelineRows }, { data: businessAreaCache }, { data: visitPriceRows }] = await Promise.all([
    ...
    detail ? Promise.resolve({ data: [] }) : supabase.from("visit_records").select("price_per_person").eq("group_place_id", groupPlace.id).is("deleted_at", null).is("hidden_at", null),
  ]);
  const place = detail ? {
  ...
  const priceSummary: PriceSummary = detail
    ? priceSummaryFromDatabase(detail.avg_price_per_person, detail.price_sample_count)
    : averageValidVisitPrices((visitPriceRows ?? []).map((row) => (row as { price_per_person?: unknown }).price_per_person));
  if (!place) notFound();
  ...
  return <AppShell groupName={context.groupName}><section className="place-detail">
    ...
    <p className="place-price-summary">{priceSummary.avgPricePerPerson === null || priceSummary.priceSampleCount < 1 ? "还没有人均记录" : `人均约 ¥${formatAveragePrice(priceSummary.avgPricePerPerson)} · 基于 ${priceSummary.priceSampleCount} 次到访记录`}</p>
