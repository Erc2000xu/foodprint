...
export default async function DiscoverPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const cards = unfilteredCards.filter(({ groupPlace, place, stat, scenes, averageEnvironment }) => {
    ...
    if (minRating && (stat.average_rating === null || Number(stat.average_rating) < Number(minRating))) return false;
  ...
  return <AppShell activeNav="发现"><section className="discover-page"><div className="discover-heading"><div><p className="eyebrow">发现</p><h1>朋友真实推荐的地方</h1></div><Link href="/map">看地图</Link></div><p className="discover-intro">已验证的推荐优先展示；完整评分会随着真实记录慢慢补齐。</p><DiscoverFilters cities={cities} selectedCategories={selectedCategories} selectedScenes={selectedScenes} city={city} minRating={minRating} minEnvironment={minEnvironment} wanted={wanted} />{cards.length ? <ul className="discover-list">{cards.map(({ groupPlace, place, stat, scenes, averageEnvironment }) => <li key={groupPlace.id}><article className="place-card"><Link href={`/place/${groupPlace.id}`} className="place-card__main"><span className="place-card__category">{categoryLabels[groupPlace.primary_category] ?? "餐饮"}</span><h2>{place.name}</h2>{place.branch_name && <p>{place.branch_name}</p>}<p>{place.address || [place.city, place.district].filter(Boolean).join(" · ") || "地址待补充"}</p>{scenes.length > 0 && <div className="place-card__scenes">{scenes.slice(0, 3).map((scene) => <span key={scene}>{sceneTagLabels[scene]}</span>)}</div>}{stat.mark_count ? <div className="place-card__stats"><strong>{Number(stat.average_rating).toFixed(1)}</strong><span>小组均分</span><b>{stat.mark_count}</b><span>人标记</span><b>{stat.recommend_count}</b><span>人推荐</span>{averageEnvironment !== undefined && <b>{averageEnvironment.toFixed(1)}</b>} {averageEnvironment !== undefined && <span>环境</span>}</div> : <div className="place-card__new"><strong>新推荐</strong><span>已由成员真实验证</span></div>}</Link><WishlistToggle groupPlaceId={groupPlace.id} initialWanted={wishlistIds.has(groupPlace.id)} /></article></li>)}</ul> : <div className="empty-state"><strong>{unfilteredCards.length ? "没有符合筛选条件的地点" : "共同地图还没有可发现的地点"}</strong><span>{unfilteredCards.length ? "换个筛选条件再试试。" : "先把想试的地方放进候选清单，体验后再推荐给朋友。"}</span>{unfilteredCards.length ? <Link className="primary-link" href="/discover">清空筛选</Link> : <Link className="primary-link" href="/try">去试试</Link>}</div>}</section></AppShell>;
}
