...
import { PlaceContentManagement, type ManagementCounts } from "@/components/admin/place-content-management";
import { ParkingAccessManagement } from "@/components/admin/parking-access-management";
...
export type AdminDeferredData = {
  ...
  managementCounts: ManagementCounts | null;
  managementCountsError: boolean;
  parkingAccess: ParkingAccess | null;
  parkingMembers: ParkingMember[];
};
...
export function AdminDeferredPanels({ data, groupId, appUrl, isOwner, isManager }: { data: Promise<AdminDeferredData>; groupId: string; appUrl?: string; isOwner: boolean; isManager: boolean }) {
  ...
  return <>
    {deferred.parkingAccess?.canManage && <ParkingAccessManagement groupId={groupId} access={deferred.parkingAccess} initialMembers={deferred.parkingMembers} />}
    {isManager && <><PlaceContentManagement counts={deferred.managementCounts} error={deferred.managementCountsError} /><DiscoveryBackfill places={incompleteDiscoveryPlaces} /><section className="admin-card"><h2>邀请朋友</h2><p>新成员验证邮箱后，可通过邀请链接加入共同地图。</p><InviteForm groupId={groupId} /></section><section className="admin-card"><h2>邀请记录</h2><p>这里只显示仍有效的邀请。请私下发送链接；失效、撤销和用完的邀请会保留在后台记录中。</p><InvitationList invitations={invitationSummaries} /></section></>}
