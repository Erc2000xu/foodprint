...
export function DiscoveryBackfill({ places }: { places: DiscoveryBackfillPlace[] }) {
  ...
  return <section className="admin-card"><h2>补充地点信息</h2><p>为历史地点补充主菜系、行政区、商圈和附近地铁；这些信息由系统自动整理。</p><ul className="discovery-backfill-list">{places.map((place) => <li key={place.groupPlaceId}><div><strong>{place.name}</strong><small>{place.address || "地址待补充"}</small></div><DiscoveryForm place={place} /></li>)}</ul></section>;
}
