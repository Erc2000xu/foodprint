...
import { rememberParkingNavigationState } from "@/lib/parking/navigation-state";
...
type MapFilterMenuProps = {
  menu: "location" | "cuisine" | "level";
  districts: AmapDistrict[];
  districtError: string;
  locationFilter?: SearchState["locationFilter"];
  availableCuisines: readonly CuisineOption[];
  draftCategoryIds: string[];
  draftCuisineIds: string[];
  draftRecommendationLevels: BowlStrength[];
  onSelectDistrict: (district: AmapDistrict) => void;
  onClearLocation: () => void;
  onToggleCategory: (id: string) => void;
  onToggleCuisine: (id: string) => void;
  onToggleLevel: (level: BowlStrength) => void;
  onClearDraft: () => void;
  onCommitDraft: () => void;
};
...
function MapFilterMenu({
  ...
  if (menu === "location") {
    return <div className="map-filter-menu map-filter-menu--location" id="map-filter-menu-location" role="dialog" aria-label="地区筛选" onPointerDown={stopGesture} onWheel={stopGesture} onTouchMove={stopGesture}><strong>选择地区</strong>{districts.map((district) => <button key={district.adcode} type="button" className={locationFilter?.id === district.adcode ? "is-selected" : ""} onClick={() => onSelectDistrict(district)}>{district.name}</button>)}{!districts.length && <small>{districtError || "正在加载行政区…"}</small>}{locationFilter && <button type="button" className="map-filter-menu__clear" onClick={onClearLocation}>清除地区</button>}</div>;
  }
  if (menu === "cuisine") {
    return <div className="map-filter-menu" id="map-filter-menu-cuisine" aria-label="餐馆类型筛选" role="dialog" onPointerDown={stopGesture} onWheel={stopGesture} onTouchMove={stopGesture}><strong>餐馆类型</strong><div className="map-filter-menu__section"><small>餐饮分类</small>{categoryOptions.map(([id, label]) => <button key={id} type="button" aria-pressed={draftCategoryIds.includes(id)} className={draftCategoryIds.includes(id) ? "is-selected" : ""} onClick={() => onToggleCategory(id)}>{label}</button>)}</div><div className="map-filter-menu__section"><small>菜系</small>{availableCuisines.map(([id, label]) => <button key={id} type="button" aria-pressed={draftCuisineIds.includes(id)} className={draftCuisineIds.includes(id) ? "is-selected" : ""} onClick={() => onToggleCuisine(id)}>{label}</button>)}</div><div className="map-filter-menu__footer"><button type="button" className="text-button" onClick={onClearDraft}>清除</button><button type="button" className="primary-button" onClick={onCommitDraft}>完成</button></div></div>;
  }
  return <div className="map-filter-menu" id="map-filter-menu-level" role="dialog" aria-label="推荐等级筛选" onPointerDown={stopGesture} onWheel={stopGesture} onTouchMove={stopGesture}><strong>推荐等级</strong>{([1, 2, 3] as const).map((level) => <button key={level} type="button" aria-pressed={draftRecommendationLevels.includes(level)} className={draftRecommendationLevels.includes(level) ? "is-selected" : ""} onClick={() => onToggleLevel(level)}>{recommendationLabels[level]}</button>)}<div className="map-filter-menu__footer"><button type="button" className="text-button" onClick={onClearDraft}>清除</button><button type="button" className="primary-button" onClick={onCommitDraft}>完成</button></div></div>;
}
...
export function DiscoveryBrowser({ places, cuisineOptions: availableCuisines, userId = "anonymous", canManage = false, canUseParking = false, indexStatus = "empty", mapRuntimeConfig = { enabled: false } }: DiscoveryBrowserProps) {
  ...
  const [filterPopoverLeft, setFilterPopoverLeft] = useState(0);
  ...
  const filterPopoverLayerRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const closeOnOutsidePointer = (event: PointerEvent) => {
      ...
      if (openMenu && !interactionRootRef.current?.contains(target) && !listIntentActionsRef.current?.contains(target)) {
        if (menuHistoryRef.current) window.history.back();
        else setOpenMenu(undefined);
      }
    };
  ...
  useLayoutEffect(() => {
    ...
    const updatePopoverPosition = () => {
      const layer = filterPopoverLayerRef.current;
      const trigger = triggerRefs.current[openMenu];
      if (!layer || !trigger) return;
      const layerRect = layer.getBoundingClientRect();
      const triggerRect = trigger.getBoundingClientRect();
      const requestedWidth = openMenu === "location" ? 230 : 330;
      const availableWidth = Math.min(requestedWidth, layerRect.width);
      const maxLeft = Math.max(0, layerRect.width - availableWidth);
      const unshiftedLayerLeft = layerRect.left - filterPopoverLeft;
      const desiredLeft = triggerRect.left - unshiftedLayerLeft;
      setFilterPopoverLeft(Math.min(maxLeft, Math.max(0, desiredLeft)));
    };
  ...
  useEffect(() => () => {
    ...
    if (menuHistoryRef.current) {
      ...
      if (currentState && typeof currentState === "object") {
        const rest = { ...(currentState as Record<string, unknown>) };
        delete rest.__foodprintFilterMenu;
  ...
  useEffect(() => {
    ...
    if (opened) {
      ...
      const frame = requestAnimationFrame(() => (document.getElementById(`map-filter-menu-${opened}`) ?? menuPanelRef.current)?.querySelector<HTMLElement>("button, [tabindex='0']")?.focus());
  ...
  const rememberMapReturnState = () => {
    ...
    if (viewport) {
      rememberParkingNavigationState(userId, {
        discoveryViewport: {
  ...
  const closeOpenMenu = (options: { preserveHistory?: boolean } = {}) => {
    if (!openMenu) return;
    if (menuHistoryRef.current && options.preserveHistory) {
      const currentState = window.history.state;
      if (currentState && typeof currentState === "object") {
        const nextState = { ...(currentState as Record<string, unknown>) };
        delete nextState.__foodprintFilterMenu;
  ...
  const openFilterMenu = (menu: OpenMenu) => {
    if (openMenu === menu) {
      closeOpenMenu();
      return;
    }
    ...
    if (!openMenu && !menuHistoryRef.current) {
      const currentState = window.history.state;
      const nextState = currentState && typeof currentState === "object" ? { ...(currentState as Record<string, unknown>), __foodprintFilterMenu: menu } : { __foodprintFilterMenu: menu };
  ...
  return <section className={`home-explorer${mapMode ? " home-explorer--map" : ""}`} aria-label="寻找地点">
    {mapMode ? <section className="dynamic-map-shell" aria-label="发现地点的动态地图" style={{ "--map-sheet-height": `${sheetHeight}px` } as CSSProperties}>
      <div ref={interactionRootRef} className="map-overlay-controls">
        <div className="map-search-stack"><form className="map-search" onSubmit={submit}><span aria-hidden="true">⌕</span><input value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="搜餐厅、区域、商圈、菜系" aria-label="搜索餐厅、区域、商圈、地铁、菜系或推荐菜" /><button type="submit" disabled={isSearchingAmap} aria-label="提交搜索">{isSearchingAmap ? "…" : "搜索"}</button></form>{canUseParking && <Link className="parking-home-entry" href="/parking" aria-label="打开停车地图" prefetch={false} onClick={rememberMapReturnState}><img src="/icons/map-controls/motorcycle-parking-button-honey-v3.png" srcSet="/icons/map-controls/motorcycle-parking-button-honey-v3-1x.png 1x, /icons/map-controls/motorcycle-parking-button-honey-v3-2x.png 2x, /icons/map-controls/motorcycle-parking-button-honey-v3-3x.png 3x" width={44} height={44} alt="" draggable={false} /></Link>}</div>
      ...
      <div className="intent-search-stack"><form className="intent-search" onSubmit={submit}><span aria-hidden="true">⌕</span><input value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="搜餐厅、区域、菜系或推荐菜" aria-label="搜索餐厅、区域、商圈、地铁、菜系或推荐菜" /><button type="submit" disabled={isSearchingAmap}>{isSearchingAmap ? "搜索中…" : "搜索"}</button><small>可搜餐厅、区域、商圈、地铁、菜系或推荐菜。</small></form>{canUseParking && <Link className="parking-home-entry parking-home-entry--list" href="/parking" aria-label="打开停车地图" prefetch={false} onClick={rememberMapReturnState}><img src="/icons/map-controls/motorcycle-parking-button-honey-v3.png" srcSet="/icons/map-controls/motorcycle-parking-button-honey-v3-1x.png 1x, /icons/map-controls/motorcycle-parking-button-honey-v3-2x.png 2x, /icons/map-controls/motorcycle-parking-button-honey-v3-3x.png 3x" width={44} height={44} alt="" draggable={false} /></Link>}</div>
