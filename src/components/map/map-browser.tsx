...
export function DiscoveryBrowser({ places, cuisineOptions }: { places: MapPlace[]; cuisineOptions: readonly CuisineOption[]; geoOptions?: GeoOption[] }) {
  ...
  return <section className="home-explorer" aria-label="找餐厅">
    ...
    {mode === "map" ? <section className="v1-static-map" aria-label="发现地点的地图视图"><StaticMapAdapter pins={filteredPlaces} selectedPlaceId={state.selectedPlaceId} onError={reportMapError} /><div><strong>{filteredPlaces.length} 家朋友推荐</strong><button type="button" onClick={() => switchView("list")}>查看列表</button></div>{mapError && <p className="map-status-message">{mapError}</p>}</section> : <section className="home-results">{!activeSearch && <div className="home-results__intro"><p className="eyebrow">朋友最近推荐</p><h2>先从这些值得去的地方开始</h2></div>}{(activeSearch ? filteredPlaces : recentPlaces).length ? <ul className="home-place-list">{(activeSearch ? filteredPlaces : recentPlaces).map((place) => { const goodTags = Object.entries(place.goodTagCounts ?? {}).filter(([, count]) => count > 0).slice(0, 2); return <li key={place.id}><div className="home-place-card-wrap"><Link href={detailHref(place.id)} className="home-place-card"><div className="home-place-card__photo">{place.coverPhotoUrl ? <img src={place.coverPhotoUrl} alt={`${place.name} 的真实照片`} /> : <span>食迹<br />推荐</span>}</div><div><div className="home-place-card__meta"><span>{place.cuisineSlugs?.map((slug) => cuisineLabelBySlug[slug]).filter(Boolean).slice(0, 1).join(" · ") || categoryLabels[place.category] || "餐饮"}</span>{place.district && <span className="location-tag location-tag--district">行政区 · {displayAmapAdministrativeLocation(place.city, place.district)}</span>}</div><h2>{place.name}</h2><p className="home-place-card__location">{locationFilter?.kind === "metro_station" ? `近 ${locationFilter.name} · ` : ""}{place.pricePerPerson ? `人均 ¥${Math.round(place.pricePerPerson)}` : "人均待补充"}</p>{place.bowlStrength ? <div className="home-place-card__score-line"><BowlIcon level={toBowlLevel(place.bowlStrength)} size="md" /><span>{place.bowlStrength} 层小碗 · {place.markCount} 位朋友吃过</span></div> : place.markCount ? <div className="home-place-card__score-line"><b>{place.averageRating.toFixed(1)}</b><span>{place.markCount} 位朋友标记</span></div> : <div className="home-place-card__score-line"><b>新推荐</b><span>已由成员真实验证</span></div>}{goodTags.length > 0 ? <div className="home-place-card__tags">{goodTags.map(([slug, count]) => <span key={slug}>{({ tasty: "吃得香", comfortable: "坐得住", good_for_chat: "聊得开", good_value: "花得值" } as Record<string, string>)[slug] ?? slug} · {count}</span>)}</div> : place.sceneTags.length > 0 && <div className="home-place-card__tags">{place.sceneTags.slice(0, 2).map((slug) => <span key={slug}>{sceneTagLabels[slug] ?? slug}</span>)}</div>}{place.recommendedItems?.length ? <p className="home-place-card__recommend">推荐：{place.recommendedItems.slice(0, 2).join("、")}</p> : place.review ? <p className="home-place-card__recommend">{place.review}</p> : null}</div></Link><WishlistToggle groupPlaceId={place.id} initialWanted={Boolean(place.savedForLater)} /></div></li>; })}</ul> : <div className="empty-state"><strong>{places.length ? "当前条件下暂无朋友推荐" : "共同地图还没有真实标记"}</strong><span>{places.length ? "清除筛选或扩大范围再试试。" : "从添加第一家真实体验开始。"}</span>{places.length ? <button className="text-button" type="button" onClick={clearAll}>清除筛选</button> : <Link className="primary-link" href="/mark">去标记地点</Link>}</div>}</section>}
  </section>;
}
