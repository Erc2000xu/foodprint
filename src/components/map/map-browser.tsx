...
function locationKindLabel(kind: DiscoveryLocationFilter["kind"]) { return kind === "district" ? "行政区" : kind === "metro_station" ? "地铁" : "商圈"; }
...
export function DiscoveryBrowser({ places, cuisineOptions, canManage = false }: { places: MapPlace[]; cuisineOptions: readonly CuisineOption[]; geoOptions?: GeoOption[]; canManage?: boolean }) {
  ...
  const reportMapError = useCallback(() => setMapError("地图暂时无法显示；你仍可在列表中查找地点。"), []);
  const requestNearby = () => {
    if (!navigator.geolocation) { setLocationMessage("此浏览器不支持定位；你仍可按推荐或最近体验查看。"); return; }
    setLocationMessage("正在获取位置…");
    navigator.geolocation.getCurrentPosition(({ coords }) => { setOrigin({ latitude: coords.latitude, longitude: coords.longitude }); setLocationMessage("已按离你最近排序；位置仅用于本次排序。"); commit({ sort: "distance" }); }, () => setLocationMessage("未获得定位权限；你仍可按推荐或最近体验查看。"), { enableHighAccuracy: false, timeout: 10_000, maximumAge: 300_000 });
  };
  ...
  return <section className="home-explorer" aria-label="寻找地点">
    <header className="home-explorer__header"><div><p className="eyebrow">发现 · 朋友吃过的地方</p><h1 className="creative-title">今天想去哪儿吃？</h1><p>从朋友吃过的地方里，选一家合适的。</p></div><div className="map-view-toggle" role="group" aria-label="切换发现列表或地图"><button className={mode === "list" ? "is-active" : ""} type="button" onClick={() => switchView("list")}>列表</button><button className={mode === "map" ? "is-active" : ""} type="button" onClick={() => switchView("map")}>地图</button></div></header>
