...
export function MealRecordForm({ groupPlaceId, placeName, currentOpinion }: { groupPlaceId: string; placeName: string; currentOpinion: CurrentOpinion }) {
  ...
  if (state.success) return <section className="mark-card mark-success-card"><p className="eyebrow">这一顿记好了</p><h1>{placeName}</h1><p className="form-success">{state.success}</p>{state.warning && <p className="form-error">{state.warning}</p>}<Link className="primary-link" href={`/place/${groupPlaceId}`}>回到地点详情</Link></section>;

  return <section className="mark-card"><p className="eyebrow">记一顿</p><h1>{placeName}</h1><p>把这次感受留在地点记录里；每次推荐只按你当前的观点计算。</p><form className="mark-form" action={action} onSubmit={(event) => { if (photosProcessing) event.preventDefault(); }}>
