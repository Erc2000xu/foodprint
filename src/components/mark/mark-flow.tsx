...
export function MarkFlow({ apiKey }: { apiKey?: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void loadAmap(apiKey, ["AMap.PlaceSearch"]).then((AMap) => {
        ...
        search.search(keyword.trim(), (status: string, result: { poiList?: { pois?: Array<{ id?: string; name?: string; address?: string; cityname?: string; adname?: string; location?: { lng: number; lat: number } }> } }) => {
          ...
          if (status !== "complete") { setResults([]); setSearching(false); return; }
          setResults((result.poiList?.pois ?? []).flatMap((poi) => poi.id && poi.name && poi.location ? [{ poiId: poi.id, name: poi.name, address: poi.address ?? "", city: poi.cityname ?? "", district: poi.adname ?? "", latitude: poi.location.lat, longitude: poi.location.lng }] : []));
          setSearching(false);
        });
      }).catch(() => { if (currentRequest === requestId.current) { setResults([]); setSearching(false); } });
    }, 420);
